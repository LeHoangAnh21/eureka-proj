...
interface Customer { id: string; name: string; code: string; type: string; }
...
function PageContent() {
  ...
  const [lines, setLines] = useState<SOLine[]>([{ productId: '', orderedQty: 1, unitPrice: 0 }]);
  ...
  const openCreate = async () => {
    setCreateOpen(true);
    setFormError('');
    setCustomerId(''); setWarehouseId(''); setCurrencyId('');
    setOrderDate(new Date().toISOString().slice(0, 10));
    setNotes('');
    setLines([{ productId: '', orderedQty: 1, unitPrice: 0 }]);
    try {
      const [c, w, cur, p] = await Promise.all([
        api.get<{ data: Customer[] }>('/partners?type=CUSTOMER&limit=200'),
  ...
  const updateLine = (i: number, field: keyof SOLine, value: string | number) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/sales-orders', {
        customerId, warehouseId, currencyId,
        orderDate, notes,
        lines: lines.map((l) => ({
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Đơn Xuất Hàng</h1>
          <p className="text-sm text-gray-500 mt-0.5">Quản lý Sales Orders</p>
        </div>
        {canCreate && (
          <button
            onClick={openCreate}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Mã đơn', 'Khách hàng', 'Kho', 'Trạng thái', 'Ngày đặt', ''].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {orders.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-12 text-gray-400">Chưa có đơn xuất nào</td></tr>
              ) : orders.map((o) => (
                <tr
                  key={o.id}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      onClick={(e) => { e.stopPropagation(); router.push(`/sales-orders/${o.id}`); }}
                    >Chi tiết</button>
                  </td>
                </tr>
              ))}
      ...
      {/* Create Modal */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Tạo đơn xuất hàng" size="lg">
